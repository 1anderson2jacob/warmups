...
const fetchPeopleWithPromises = () => {
  return superagent
    ...
    .then(peopleRequests => {
      return Promise.all(peopleRequests).then(people => {
        let names = [];
...
const fetchPeopleWithAsync = async () => {
  ...
  const names = await Promise.all(peopleRequests).then(people => {
    let names = [];
    for (let data of people) {
      names.push(data.body.name);
    }
...
fetchPeopleWithAsync().then(people => console.log({ people }));
